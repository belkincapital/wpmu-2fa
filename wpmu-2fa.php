...
----------------------------------------------------------------------------
    ...
    Thanks to Henrik Schack for his Google Authenticator v0.47 plugin.
	...
	for ($i=$firstcount; $i<=$lastcount; $i++) {
		...
		if ( $value === $thistry ) {
			...
			if ( $lasttimeslot >= ($tm+$i) ) {
				error_log("2FA Notice: Man-in-the-middle attack detected (Could also be 2 legit login attempts within the same 30 second period)");
				return false;
			}
			...
			if ( trim( get_user_option( 'googleauthenticator_pwdenabled', $user->ID ) ) == 'enabled' && ( defined('XMLRPC_REQUEST') || defined('APP_REQUEST') ) ) {
				...
				if ( $passwordhash == $usersha1 ) { // ToDo: Remove after some time when users have migrated to new format
				  ...
				  // Try the new version based on thee wp_hash_password	function
					...
					return new WP_Error( 'invalid_google_authenticator_password', __( '<strong>ERROR</strong>: The 2FA code is incorrect.', 'google-authenticator' ) );
				} 		 
			} else {
				return new WP_Error( 'invalid_google_authenticator_token', __( '<strong>ERROR</strong>: The 2FA code is incorrect or has expired.', 'google-authenticator' ) );
			}	
		}
	}
...
function profile_personal_options() {
	...
	echo "<h3>".__( 'Two-Factor Authentication', 'google-authenticator' )."</h3>\n";
        echo "Two-Factor Authentication (2FA) allows you to secure your account so no one can login even if they steal your password. In order to use 2FA, you must install the <b>Google Authenticator</b> app onto your phone or tablet. It is available either via the <a target='_blank' href='https://itunes.apple.com/us/app/google-authenticator/id388497605'>Apple AppStore</a> or <a target='_blank' href='https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2'>Google Play</a>.";
	echo "<table class=\"form-table\">\n";
	echo "<tbody>\n";
	echo "<tr>\n";
	echo "<th scope=\"row\">".__( 'Active', 'google-authenticator' )."</th>\n";
	echo "<td>\n";
	echo "<input name=\"GA_enabled\" id=\"GA_enabled\" class=\"tog\" type=\"checkbox\"" . checked( $GA_enabled, 'enabled', false ) . "/><span class=\"description\"><b>Important</b>: Do not set 2FA to Active until you have set it up on Google Authenticator.</span>\n";
	echo "</td>\n";
	echo "</tr>\n";
	...
	if ( $is_profile_page || IS_PROFILE_PAGE ) {
		...
		echo "<td>\n";
		echo "<input name=\"GA_relaxedmode\" id=\"GA_relaxedmode\" class=\"tog\" type=\"checkbox\"" . checked( $GA_relaxedmode, 'enabled', false ) . "/><span class=\"description\">".__('<b>Relaxed mode</b>: Only enable this option if you are running out of time when entering 2FA codes on login.','google-authenticator')."</span>\n";
		echo "</td>\n";
		echo "</tr>\n";
		...
		echo '<span class="description"><br/> Scan this with the <b>Google Authenticator</b> app available either via the <a target="_blank" href="https://itunes.apple.com/us/app/google-authenticator/id388497605">Apple AppStore</a> or <a target="_blank" href="https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2">Google Play</a>.</span>';
		echo "</div></td>\n";
		echo "</tr>\n";
